SELECT
    ...
    coalesce(anc.using_medication, pnc.using_medication, child.using_medication) AS using_medication,
    coalesce(
        anc.medication_used_currently, pnc.medication_used_currently, child.medication_used_currently
    ) AS medication_used_currently,
    coalesce(anc.source_medicine, pnc.source_medicine, child.source_medicine) AS source_medicine,
    coalesce(
        anc.not_get_all_meds_reason, pnc.not_get_all_meds_reason, child.not_get_all_meds_reason
    ) AS not_get_all_meds_reason,
    coalesce(
        anc.not_get_all_meds_reason_other, pnc.not_get_all_meds_reason_other, child.not_get_all_meds_reason_other
    ) AS not_get_all_meds_reason_other,
    coalesce(
        anc.expensive_medication_list, pnc.expensive_medication_list, child.expensive_medication_list
    ) AS expensive_medication_list,
    coalesce(
        anc.out_of_stock_medication_list, pnc.out_of_stock_medication_list, child.out_of_stock_medication_list
    ) AS out_of_stock_medication_list,
    coalesce(
        anc.other_reason_medication_list, pnc.other_reason_medication_list, child.other_reason_medication_list
    ) AS other_reason_medication_list,
    coalesce(anc.get_all_meds, pnc.get_all_meds, child.get_all_meds) AS get_all_meds,
    CASE
        ...
        WHEN anc.id IS NOT null THEN anc.source_selection_medicine
        WHEN pnc.id IS NOT null THEN pnc.source_selection_medicine
